...
flowchart TD
    ...
    Solo --> Click{"Є доріжка Click?"}
    Click -->|"Так"| ClickCheck["Прослухати Click і позначити<br/>не включати до залового міксу"]
    Click -->|"Ні"| Live
    ClickCheck --> Live{"Основний вокал і жива<br/>акустична гітара відсутні?"}
...
### 4. Окремо перевір доріжку `Click`

Поточна конфігурація цього проєкту **не передбачає кліку**: немає навушників, а заловий stereo backing track не повинен містити відлік. Тому `Click` не може мати статус `Залишити` без окремого підтвердження, що це не звуковий клік.

1. Якщо у списку є track із назвою `Click`, натисни його `S` (`Solo`) і прослухай короткий фрагмент там, де він має звучати.
2. Якщо це чутний метроном / відлік, запиши `Click: Не включати до stereo backing track`.
3. Вийди із `Solo`; перед продовженням кнопка `S` має бути вимкнена.
4. Не видаляй доріжку і не змінюй аранжування. Як виключити її лише з export, буде окремим контрольованим кроком у документі про export.
5. Якщо під назвою `Click` інший творчий елемент, запиши його фактичне призначення і познач `Перевірити`.

**Стоп-умова:** поки справжній клік позначений `Залишити`, результат не може бути `Ready for stereo export`.

### 5. Підтверди виключення живих партій
...
### 7. Прийми рішення `Ready` або `Blocked`

Постав один із результатів у нижній частині нотатки:

| Результат | Коли обирати | Наступна дія |
| --- | --- | --- |
| `Ready for stereo export` | Доріжки зрозумілі, справжній `Click` виключений, живий вокал і акустична гітара відсутні, межі пісні та темп зафіксовані | Перейти до [експорту stereo backing track](11-logic-export-stereo-backing-track.md) |
...
Click: Немає / Не включати до stereo backing track / Перевірити: ...
...
| `Click` позначений `Залишити` | Прослухати його через `Solo`; якщо це метроном / відлік, позначити `Не включати до stereo backing track` |
