...
```mermaid
flowchart TD
    Q["Технічне питання"] --> B["Brave: знайти офіційне джерело"]
    B --> O{"Офіційне джерело<br/>підтверджує крок?"}
    O -->|"Так"| W["Написати крок + URL + дата перевірки"]
    O -->|"Ні"| T["Позначити: Офіційно не підтверджено"]
    T --> H["Створити безпечний домашній тест"]
    W --> P{"Потрібен досвід<br/>інших виконавців?"}
    P -->|"Так"| R["Окремий Brave-пошук практичних прикладів"]
    P -->|"Ні"| D["Документ готовий"]
    R --> D
```
