...
- Додатковий screenshot показав, що в macOS Tahoe іконка Bluetooth endpoint AIRSTEP Lite не має видимих In/Out arrows, тож Apple visual test не дає окремого signal indicator для цього конкретного device view. MainStage також не отримав MIDI learn, тому наступний evidence-based крок — inspection current preset у офіційному AIRSTEP App (без редагування). XSONIC/App Store підтверджують, що editor визначає MIDI/HID message parameters та передає preset на Lite: [AIRSTEP app](https://apps.apple.com/pl/app/airstep/id1487765085?l=pl), перевірено 2026-07-26.

## 2026-07-26 — уточнення preset AIRSTEP Lite з офіційного застосунку

- Screenshot офіційного застосунку AIRSTEP показує `MIDI PC`, `AirStep Local Preset 1` та у панелі **LITE**: `A = PC 0`, `B = PC 1`, `C = PC 2`, `D = PC 3`, `E = PC 4`; у всіх рядках зазначено `MIDI` і `1 Msg`. Це відкидає попередню можливість, що показана кнопка A є HID keyboard command.
- Первинно `Not Connected` було помилково віднесено до Lite. Повторний аналіз screenshot показав, що напис належить верхній секції **AIRSTEP**; саме вона містить `MIDI IN`, `EXP1` та `EXP2`. За офіційною специфікацією XSONIC Lite не має цих interfaces. Нижня секція **LITE** має власні switches, battery та `AirStep Local Preset`; користувач підтвердив, що `Fn` фізично перемикає ці позиції. Отже Lite не варто вважати непідключеною або несправною через верхній status.
- Офіційний [XSONIC AIRSTEP User Manual](https://help.xsonicaudio.com/user_manual/1) описує підключення editor через `Touch to Connect AIRSTEP` та вибір пристрою; він також зазначає, що з firmware 1.7+ можна мати до трьох Bluetooth connections. Безпечна наступна дія — під’єднати app до Lite, не від’єднуючи MacBook автоматично, й підтвердити active preset.
- `Not Connected` є статичним текстом у верхній секції AIRSTEP, не connection control та не status Lite. Виявлений у section 10 reset preset через app (`Save` → `DONE`) не є підтвердженим physical factory reset Lite. Його не застосовуємо: причина для reset (нібито `Not Connected` Lite) відпала.

## 2026-07-26 — перевірка досвіду користувачів щодо factory reset AIRSTEP Lite

- Пошук обговорень користувачів не виявив незалежно підтвердженої фізичної комбінації кнопок **саме для AIRSTEP Lite**. Практичний матеріал, що повторюється, збігається з описом XSONIC: reset preset здійснюється через app після connection, а не кнопками на Lite.
- Знайдена порада `hold 4th footswitch` або `FUNC + C` під час увімкнення належить **AIRSTEP Kat / Kat EX Edition**, а не Lite; сам автор обговорення зазначає, що ці способи йому не допомогли. Це недостатній і нерелевантний доказ, тому процедура категорично не переноситься на Lite: [обговорення Reddit](https://www.reddit.com/r/BossKatana/comments/1r0yra5/air_step_kat_working_strange/), перевірено 2026-07-26.
- Знайдений physical gesture для Lite — утримання правої кнопки `E` під час увімкнення — перемикає **standalone / wireless-extension mode**, не скидає presets. Його не використовуємо як reset: [XSONIC User Manual mirror/index](https://www.docmeeting.com/rcex.php?Embedded=1&RCD_ID=50555&docQuery=command&querida=3), перевірено 2026-07-26.
- Висновок: користувацькі джерела не додають безпечного physical factory reset. Для поточного проєкту дозволений лише документований шлях через connected app або точна відповідь XSONIC Support для serial/firmware пристрою.

## 2026-07-26 — рішення щодо source messages AIRSTEP Lite для MainStage

- Firmware 1.9.7 і screenshot показують, що Lite зараз надсилає `Program Change` (`PC 0`–`PC 4`). Це не дефект: XSONIC підтверджує, що app дозволяє редагувати MIDI message type і Bluetooth output interface для кожної footswitch.
- Для команд Playback обрано momentary `Control Change`, а не `Program Change`: Apple current [MainStage release notes](https://support.apple.com/en-us/101568) документують, що incoming Program Change використовується для patch selection, тоді як окремо виправлено обробку `Control Change Momentary buttons`. Отже `CC` краще ізолює команди Playback від логіки song-patch selection.
- Зарезервовано тільки для цього Concert: `CC 20` = Lite `A` / `Play`, майбутні `CC 21–24` = `B–E`. `CC 7` та `CC 10` не використовуються, бо часто застосовуються для volume і pan. Зміна виконується спочатку лише на Local Preset 1, з screenshot до зміни й без натискання global `Save` до перевірки.
- Після screenshot Local Preset 2 план скориговано на користь фактичного стану: він уже має `MIDI CC`, Lite `A = CC 22 Toggle`. Перше призначення MainStage тестує цей message без редагування педалі. Це зменшує кількість змін і окремо перевіряє, чи toggle CC запускає та зупиняє Playback.
- XSONIC вказує максимум п’ять local presets, але не документує delete окремого hardware slot. Тому slots не видаляються: після успішного test ціллю буде зробити всі п’ять однаковими, аби випадкове `Fn` не змінило stage behavior.

## 2026-07-26 — AIRSTEP Lite CC підтверджено; transport toggle відхилено

- Користувач підтвердив `AIRSTEP MIDI In: Pass`: у MainStage видно incoming MIDI activity, а `Local Preset 2`, Lite `A = CC 22 Toggle`, запускає `Play`. Це остаточно підтверджує весь ланцюг Lite → Bluetooth MIDI → macOS → MainStage assignment.
- Послідовність `A`: start + LED on; наступний press — без transport action + LED off; третій press — stop + LED on; четвертий — без action + LED off. Це саме очікувана семантика toggle: перше натискання надсилає `127`, друге `0`; незалежна офіційна документація Ableton описує цю пару значень і зазначає, що hardware LED може не відповідати стану софту ([Toggle and Momentary MIDI functions](https://help.ableton.com/hc/en-us/articles/209774945-Toggle-and-Momentary-MIDI-functions), перевірено 2026-07-26). Це не evidence hardware defect.
- XSONIC підтверджує, що footswitch можна програмувати на `press`, `release`, `long press` або toggle mode, а Lite зберігає до п’яти local presets ([XSONIC AIRSTEP](https://xsonicaudio.com/pages/airstep), розділи `Footswitches`, `AIRSTEP Lite`, `APP Editor`, перевірено 2026-07-26). Наступний мінімальний test: зберегти `CC 22`, але вимкнути toggle і зробити одне press message value `127`. Це не змінює mapping MainStage.
- Screenshot `Switch LA` підтвердив точний стан app до зміни: `Toggle On` = `CC 22, value 127`; `Toggle Off` = `CC 22, value 0`; `Trigger = Press`; `Output Interface = ALL`; `MIDI Channel = 1`. Тому інструкція більше не припускає назви полів: змінюється тільки видимий перемикач `Toggle Mode`, решта значень залишаються без змін.
- Наступний фактичний screenshot normal-mode screen виявив важливу поведінку app: перемикання `Toggle Mode` у `Off` створило normal `Message 1` з `CC Number 0` і `CC Value 0`, хоча заголовок switch лишився `CC 22`. MainStage показав саме `1 0 0`; це пояснює відсутність реакції. Інструкцію виправлено: після вимкнення toggle обов’язково вручну повернути message `CC Number 22`, `CC Value 127`, а потім зберегти edit page. Це не вимагає нового MainStage learn.
- Після відновлення `CC 22 = 127` normal press-trigger пройшов три повтори `A → start; MainStage Stop; A → start`. Статус transport A — `Pass`.
- Користувач просить LED Lite синхронізувати з playback MainStage. Перевірка джерел розділяє це від локального `LED Display`: [посібник AIRSTEP](https://shop.warwick.de/media/72/14/2a/1750079133/4106850169cb0f43_XSO_AIRSTEP_AIRSTEP_LITE_Manual_EN.pdf.pdf?ts=1750079133) описує `Normal` як індикацію поточного trigger, а `Toggle` як локальне чергування першого/другого trigger (p. 7); це не host-state feedback. XSONIC описує Lite як Bluetooth MIDI/HID **output** controller ([XSONIC specification](https://xsonicaudio.com/pages/airstep), перевірено 2026-07-26). Apple документує MIDI feedback лише для конкретних controller integrations, зокрема Arturia KeyLab, а не generic AIRSTEP Lite ([MainStage release notes](https://support.apple.com/en-us/101568), перевірено 2026-07-26). Тому зараз LED не використовується як stage-critical індикатор і не налаштовується як ніби feedback гарантовано доступний.

## 2026-07-26 — затверджено повну тестову карту AIRSTEP Lite

- На запит користувача «усі інші кнопки» карта розширена без зміни вже перевіреної A: `A = Start from beginning / CC 22`, `B = Previous Marker / CC 23`, `C = Next Marker / CC 24`, `D = Cycle / CC 25`, `E = Stop / CC 26`. Усі джерельні messages — одноразові normal `Press` із value `127`, без Toggle, Release, long press або Message 2.
- Обрано окремий `E = Stop`, а не резерв: це реалізує практичний принцип ізольованої аварійної transport-команди. CC 10 і CC 11 не використовуються; CC 10 зокрема має спеціальну MainStage Pan-логіку.
- Apple current [MainStage release notes](https://support.apple.com/en-us/101568) прямо згадують Playback `Next Marker`, `Previous Marker`, `Cycle` і виправлення mapping Playback buttons у 4.3. Маршрут прив’язує Lite до видимих screen controls template, а не покладається на непідтверджене ручне введення назви action.

## 2026-07-26 — AIRSTEP FULL ROUTE Pass; PC song selection deferred

- Користувач підтвердив `AIRSTEP FULL ROUTE: PASS — A Start, B Previous Marker, C Next Marker, D Cycle, E Stop`. Це означає, що всі п’ять action buttons підключені, не конфліктують між собою й пройшли обумовлені tests у `Backing Tracks — MARKER TEST.concert`.
- Практичне спостереження B під час playback: один press повертає до початку поточної marker-секції, два presses — до попередньої. Це відповідає опису Playback `Go to Previous Marker`: команда обирає marker ліворуч від **поточного playhead**, тому в середині секції найближчим попереднім marker є її початок ([MainStage Help mirror: Playback transport and function buttons](https://mainstage.skydocu.com/en/playback-plug-in/playback-transport-and-function-buttons/), перевірено 2026-07-26). Це secondary copy документації, не поточна офіційна Apple URL; первинним evidence для цього Concert є власний test користувача. Поведінку прийнято як safety behavior, не як універсальне твердження про всі контролери.
- Користувач явно відклав MIDI Program Change для перемикання наступної/довільної пісні. Поточний Local Preset 2 `CC 22–26` не змінюється. PC повертається в план тільки після появи кількох готових song patches та окремого research gate для set-list масштабу ~25 пісень.

## 2026-07-26 — перехід до stems для першої пісні

- Користувач явно обрав advanced workflow: окремо експортувати всі tracks `Цілуються хмари` та бачити їх окремими B-Track strips MainStage для рівня, Mute і Solo. Це змінює поточний practical focus зі stereo preparation наступної пісні на stem test першої пісні.
- Apple підтверджує export одного або кількох selected tracks як audio files, а також `Cycle Range`, automation, tail, normalization і filename pattern ([Export tracks as audio files](https://support.apple.com/guide/logicpro/export-tracks-as-audio-files-lgcpb27f70f9/mac), перевірено 2026-07-26). Для гарантованої часової alignment усі stems експортуються з одним full-song Cycle Range.
- Apple окремо документує marker information для record/bounce та команду `Navigate > Other > Export Marker to Audio File` для selected audio region ([Use marker information from audio files](https://support.apple.com/guide/logicpro/use-marker-information-lgcpadb63ff8/mac), перевірено 2026-07-26). Оскільки Apple не заявляє явно, що `Export Tracks as Audio Files` автоматично включає markers, обрано evidence-first branch: import Drum stem у duplicate STEM TEST Concert; тільки якщо markers відсутні, застосувати official marker-export command до stems.
- Scope цієї фази: independent control у MainStage, всі B-Tracks у `Out 13–14`, отже CQ-12T/FOH досі отримує один stereo mix. Окремий multichannel USB mix у CQ/FOH не припускається й потребує майбутнього routing design.

## 2026-07-26 — advanced routing і navigation design

- Apple описує `Master` як загальний level Concert, окремі `Output` channel strips — як контролі конкретних physical output, а routing кількох strips у Aux — як спосіб зберегти relative levels, але мати спільний level/pan/EQ ([Working at the Concert Level](https://help.apple.com/mainstage/mac/2.2/help/English.lproj/en/mainstage/usermanual/chapter_6_section_11.html), перевірено 2026-07-26). Тому майбутній `BT-MIX` Aux, а не `Main`, обрано єдиною routing точкою.
- Apple документує, що Playback може працювати з кількома instances у спільному `Group`, може бути розміщений на patch/set/concert level, а один instance програє один file ([Using Playback in Performance](https://help.apple.com/mainstage/mac/2.2/help/English.lproj/en/mainstage/usermanual/chapter_9_section_9.html), [Adding a Playback Plug-in](https://help.apple.com/mainstage/mac/2.2/en/mainstage/usermanual/chapter_8_section_1.html), перевірено 2026-07-26). Це підтримує 10–12-stem template.
- Allen & Heath офіційно підтверджує separate DAW output → CQ individual input в `Multitrack` mode і `Input Source = USB/SD` ([CQ multitrack streaming](https://support.allen-heath.com/hc/en-gb/articles/19338440353425-CQ-Multitrack-streaming-using-CQ), перевірено 2026-07-26). Multichannel CQ routing технічно можливий, але відкладений через ризик конфлікту з analog guitar/vocal та необхідність окремої таблиці каналів.
- Apple actions `Prev Patch` та `Next Patch` вирішують звичайне перемикання пісень. MIDI Program Change призначений для direct select patch за number, не є необхідним для next/previous ([Table of Actions](https://help.apple.com/mainstage/mac/2.2/help/English.lproj/en/mainstage/usermanual/chapter_D_section_1.html), [Working with Patches](https://help.apple.com/mainstage/mac/2.2/en/mainstage/usermanual/chapter_5_section_1.html), перевірено 2026-07-26).
- Практика live-backtrack software підтримує поділ transport і song/track preparation: приклад користувацького workflow Gig Performer має synchronized `Play/Pause/Stop` та окреме перемикання preset/file за song name ([Gig Performer Community](https://community.gigperformer.com/t/global-rackspace-player-controls-for-backing-cue-tracks-plus-tempo-map/21590), перевірено 2026-07-26). Це приклад реальної практики, не офіційна інструкція MainStage.
